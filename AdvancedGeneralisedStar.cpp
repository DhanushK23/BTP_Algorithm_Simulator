...
// Advanced Generalised Star Query Model.
...
void populate2rounds(vector<vector<double> >& dp, int k)
{
    ...
    for(int i=1;i<k;i++)
    {
        double term1 = static_cast<double>((k*i))/static_cast<double>(k+i);
        double term2 = k - i;
...
void populateRrounds(vector<vector<double> >& dp, int r, int k)
{
    ...
    {
        double x = i;
        double term1 = dp[r-1][k]*x / (dp[r-1][k] + x);
